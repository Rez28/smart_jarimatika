...
function updateProgress() {
    ...
    const opponentPercent = Math.min(100, Math.round((opponentScore / total) * 100));
...
function flashResult(success) {
    ...
    flashOverlay.style.background = success ? 'rgba(56, 189, 248, 0.3)' : 'rgba(239, 68, 68, 0.3)';
...
function endBattle() {
    ...
    const winner = userScore === opponentScore ? 'Seri' : userScore > opponentScore ? 'Kamu Menang!' : 'Lawan Menang';
...
function bindPusher() {
    ...
    if (!pusherKey) {
        setOpponentStatus('Pusher tidak dikonfigurasi. Sinkronisasi real-time nonaktif.', '#f59e0b');
        logBattle('Pusher tidak tersedia pada environment. Jalankan backend broadcast untuk realtime.');
        return;
    }
...
function sendScoreToServer(points) {
    ...
    fetch(scoreUrl, {
        ...
        .then((data) => {
            if (!data.success) {
                logBattle(`Score submit gagal: ${data.message || 'unknown error'}`);
            }
        })
        .catch((error) => {
...
answerButton?.addEventListener('click', () => {
    ...
    if (detected === 0) {
        logBattle('Tidak ada angka terdeteksi. Pastikan jarimu jelas di kamera.');
