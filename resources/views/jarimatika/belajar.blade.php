...
<style>
    ...
    /* Memaksa font Fredoka dan background pattern ceria */
    .jarimatika-container {
        font-family: 'Fredoka', sans-serif;
        background-color: #FFFBEB; /* Krem sangat muda */
        background-image: radial-gradient(#FFE52A 1.5px, transparent 1.5px);
        background-size: 30px 30px;
        min-height: 100vh;
    }

    /* Efek Cermin untuk Kamera */
    .output_canvas {
        transform: scaleX(-1);
    }
...
<div class="jarimatika-container py-8 px-4 sm:px-6">
        ...
        <main class="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-8">
            ...
            <section class="flex flex-col gap-6">
                <div class="game-card p-4 border-b-[8px] border-[#BBCB64]">
                    <div class="relative w-full aspect-[4/3] bg-slate-800 rounded-2xl overflow-hidden border-4 border-slate-700 shadow-inner">
                        <video class="input_video absolute inset-0 opacity-0 pointer-events-none" autoplay muted playsinline></video>
                        <canvas class="output_canvas w-full h-full object-cover" width="640" height="480"></canvas>
                        
                        <div id="flash-effect" class="absolute inset-0 bg-white opacity-0 pointer-events-none transition-opacity duration-200 z-10"></div>
                        
                        <div id="feedback-overlay" class="absolute inset-0 flex items-center justify-center opacity-0 transition-opacity duration-300 z-20">
                            <div class="bg-white/90 p-6 rounded-full border-8 border-[#BBCB64] shadow-2xl scale-150">
                                <span class="text-6xl">🌟</span>
                            </div>
                        </div>

                        <div id="camera-off-placeholder" class="absolute inset-0 flex flex-col items-center justify-center bg-slate-800 text-slate-400 z-0">
                            <span class="text-6xl mb-4">📷</span>
                            <p class="font-semibold text-lg">Kamera belum menyala</p>
                        </div>
                    </div>

                    <div class="mt-6 flex items-center justify-between bg-slate-50 p-4 rounded-2xl border-2 border-slate-100">
                        <button id="btn-camera-toggle" class="btn-3d-orange px-6 py-3 rounded-2xl text-white font-bold flex items-center gap-2">
            ...
            <section class="flex flex-col gap-6">
                <div class="game-card p-8 border-b-[8px] border-[#38BDF8] text-center flex flex-col items-center justify-center min-h-[250px]">
                    <span class="bg-[#38BDF8] text-white px-4 py-1 rounded-full text-sm font-bold uppercase tracking-widest mb-4 inline-block">Instruksi</span>
                ...
                <div class="game-card p-6 border-b-[8px] border-slate-200 flex-grow flex flex-col">
                    ...
                    <div id="gallery-list" class="flex gap-4 overflow-x-auto pb-4 mb-4 flex-grow items-start min-h-[120px]">
                    ...
                    <a href="{{ route('jarimatika.latihan') }}" id="btn-next-level" class="hidden btn-3d-green w-full block text-center text-white text-xl font-bold rounded-2xl py-4 mt-auto">
    ...
    <div id="start-overlay" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-md px-4">
        <div class="bg-white p-10 rounded-[40px] border-b-[12px] border-[#BBCB64] max-w-lg w-full text-center shadow-2xl relative overflow-hidden">
            ...
            <p class="text-lg text-slate-600 font-medium mb-8 relative z-10">Di mode ini, kita akan belajar bagaimana bentuk jari untuk angka 1 sampai 10. Sudah siap?</p>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Fungsi Simulasi Jepret Foto (Dipanggil oleh jarimatika-game.js saat jawaban benar)
        window.snapPhoto = function(numberLabel) {
            // Memastikan fungsi crop dari core.js tersedia
            const imageData = typeof window.captureHandSmart === 'function' ? window.captureHandSmart() : null;
            if (!imageData) return;
            ...
            `;
            // Tambahkan ke paling depan list
            ...
            // 5. Tampilkan Tombol Lanjut (Setelah foto pertama berhasil)
            if (btnNextLevel) {
                btnNextLevel.classList.remove('hidden');
            }
        };

        // Loop untuk update UI angka yang terbaca oleh kamera secara real-time
        setInterval(function() {
            if (window.gameState && typeof window.gameState.detectedNumber !== 'undefined') {
                userAnswer.textContent = window.gameState.detectedNumber;
            }
        }, 150); // Dipercepat jadi 150ms agar lebih responsif di UI
    });
</script>
@endsection 