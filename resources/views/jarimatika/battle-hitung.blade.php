...
@section('content')
    <style>
        ...
        .btn-3d-purple:active:not(:disabled) {
    ...
    <div id="battle-screen" data-game-id="{{ $gameId }}" data-user-id="{{ auth()->id() }}"
        ...
        <div class="max-w-[1400px] mx-auto">
            ...
            <div
                class="game-card mb-6 px-6 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b-[8px] border-[#8B5CF6]">
                <div class="flex items-center gap-3">
                    <span
                        class="bg-[#8B5CF6] text-white px-4 py-2 rounded-xl text-lg font-bold uppercase tracking-wider shadow-sm">
                        🧮 Battle Hitung
                    </span>
                ...
                <button id="btn-camera-switch" type="button"
                    class="ml-3 px-4 py-2 bg-[#8B5CF6] text-white rounded-xl font-bold hover:bg-[#6D28D9] transition-all">Nyalakan
                    Kamera</button>
            </div>
            ...
            <main class="grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr] gap-6 items-start">
                ...
                <section class="flex flex-col gap-6">
                    <div class="game-card p-4 md:p-6 border-b-[8px] border-[#8B5CF6] relative flex flex-col">
                        <div class="flex justify-between items-center mb-4">
                            <h2 class="text-2xl font-black text-[#8B5CF6]">Kamera Kamu</h2>
                            <span class="bg-[#8B5CF6] text-white text-xs font-bold px-3 py-1 rounded-full">Player 1</span>
                        </div>

                        <div
                            class="relative w-full bg-slate-900 rounded-2xl overflow-hidden border-4 border-[#8B5CF6] aspect-video shadow-inner">
                        ...
                        <div class="mt-4 grid grid-cols-2 gap-4">
                            <div class="bg-slate-50 border-2 border-slate-200 rounded-2xl p-4 text-center">
                                ...
                                <p id="detected-number" class="text-5xl font-black text-[#8B5CF6] drop-shadow-sm mt-1">0</p>
                            </div>
                            <div class="bg-purple-100 border-2 border-purple-300 rounded-2xl p-4 text-center">
                                <p class="text-purple-700 font-bold uppercase tracking-widest text-xs">Skor Kamu</p>
                                <p id="user-score" class="text-5xl font-black text-purple-600 drop-shadow-sm mt-1">0</p>
                            </div>
                        </div>
                    </div>
                </section>
                ...
                <section class="flex flex-col gap-4 lg:min-w-[300px] z-10 relative">
                    ...
                    <div
                        class="game-card p-6 border-b-[8px] border-[#8B5CF6] text-center bg-gradient-to-br from-purple-50 to-white shadow-xl relative overflow-hidden">
                        <div class="absolute top-0 left-0 w-full h-2 bg-[#8B5CF6]"></div>
                        <p class="text-slate-500 font-bold uppercase tracking-widest text-sm mb-2">Soal Matematika</p>
                        <p id="math-problem" class="text-4xl leading-none font-black text-slate-800 drop-shadow-md mb-2">? +
                            ? = ?</p>
                        <p class="text-slate-600 font-bold text-lg">Tunjukkan dengan jari!</p>
                    </div>
                    ...
                    <div class="game-card p-4 border-b-[6px] border-slate-800 text-center bg-slate-800">
                        ...
                        <p id="battle-timer" class="text-4xl font-black text-[#8B5CF6] drop-shadow-md animate-pulse">20s</p>
                    </div>

                    <!-- Status Ready Indicator -->
                    <div class="game-card p-3 border-b-[4px] border-purple-300 text-center bg-purple-50">
                        <p class="text-slate-600 font-bold uppercase tracking-widest text-xs mb-1">Status Pemain</p>
                        <p id="ready-status" class="text-lg font-black text-purple-600">0/2 Siap</p>
                    </div>

                    <div class="flex flex-col gap-3 mt-2">
                        <button id="btn-start-battle" type="button"
                            class="btn-3d-purple w-full py-4 text-xl font-bold text-white rounded-2xl uppercase tracking-wider">
                            ✋ Ready
                        </button>
                ...
                <section class="flex flex-col gap-6">
                    <div class="game-card p-4 md:p-6 border-b-[8px] border-purple-400 relative flex flex-col">
                        <div class="flex justify-between items-center mb-4">
                            <span class="bg-purple-400 text-white text-xs font-bold px-3 py-1 rounded-full">Player 2</span>
                            <h2 id="opponent-name-display" class="text-2xl font-black text-purple-600">Lawanmu</h2>
                        </div>
                        ...
                        <div
                            ...
                            <div class="absolute inset-0 opacity-10"
                                style="background-image: radial-gradient(#8B5CF6 2px, transparent 2px); background-size: 15px 15px;">
                            </div>
                        ...
                        <div class="mt-4 bg-purple-100 border-2 border-purple-300 rounded-2xl p-4 text-center">
                            <p class="text-purple-700 font-bold uppercase tracking-widest text-xs">Skor Lawan</p>
                            <p id="opponent-score" class="text-5xl font-black text-purple-600 drop-shadow-sm mt-1">0</p>
                        </div>
                    </div>
                </section>
            </main>
                ...
                <div class="game-card p-6 border-b-[8px] border-slate-300">
                    ...
                    <div class="mb-6">
                        <div class="flex items-center justify-between mb-2">
                            <span class="font-bold text-[#8B5CF6] uppercase tracking-wide">Kamu</span>
                        ...
                        <div class="h-6 bg-slate-100 rounded-full border-2 border-slate-200 overflow-hidden">
                            <div id="user-progress"
                                class="h-full bg-[#8B5CF6] w-0 transition-all duration-500 rounded-full"></div>
                        </div>
                    </div>
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <span class="font-bold text-purple-400 uppercase tracking-wide">Lawan</span>
                            <span id="opponent-progress-text" class="font-bold text-slate-600">0 Poin</span>
                        </div>
                        <div class="h-6 bg-slate-100 rounded-full border-2 border-slate-200 overflow-hidden">
                            <div id="opponent-progress"
                                class="h-full bg-purple-400 w-0 transition-all duration-500 rounded-full"></div>
                        </div>
                    </div>
                </div>
            ...
            <button id="btn-exit-room" type="button"
                class="btn-3d-red w-full py-4 text-xl font-bold text-white rounded-2xl uppercase tracking-wider transition-all mt-6">
                Keluar
            </button>
        </div>
    </div>
        ...
        <div class="bg-white rounded-3xl p-8 max-w-md w-full mx-4 shadow-2xl">
            ...
            <div class="flex flex-col gap-4">
                <button id="btn-rematch" type="button"
                    class="btn-3d-purple w-full py-3 text-lg font-bold text-white rounded-2xl uppercase tracking-wider">Main
                    Lagi</button>
                <a href="/dashboard"
    ...
    <script src="{{ asset('js/jarimatika-battle-hitung.js') }}"></script>
@endsection
