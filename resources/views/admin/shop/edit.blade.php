...
@section('content')
    ...
    <div class="admin-container py-8 px-4 sm:px-6">
        <div class="max-w-2xl mx-auto">
            ...
            <!-- Header -->
            <div class="mb-8">
                <h1 class="text-4xl font-black text-slate-800 mb-2">✏️ Edit Item</h1>
                <p class="text-slate-600 font-semibold">Perbarui informasi item toko</p>
            </div>
            ...
            <!-- Edit Form -->
            <form action="{{ route('admin.shop.update', $item->id) }}" method="POST" enctype="multipart/form-data"
                class="form-card">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select id="type" name="type" class="form-select" required>
                        ...
                        <option value="avatar" {{ old('type', $item->type) === 'avatar' ? 'selected' : '' }}>👤 Avatar
                        </option>
                        <option value="border" {{ old('type', $item->type) === 'border' ? 'selected' : '' }}>🖼️ Border
                        </option>
                        <option value="badge" {{ old('type', $item->type) === 'badge' ? 'selected' : '' }}>🎖️ Badge
                        </option>
                        <option value="effect" {{ old('type', $item->type) === 'effect' ? 'selected' : '' }}>✨ Effect
                        </option>
                    </select>
                    @error('type')
                        <div class="error-message">{{ $message }}</div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="price" name="price" class="form-input"
                        value="{{ old('price', $item->price) }}" min="1" required>
                    @error('price')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    @if ($item->image_path)
                        <div>
                            <p class="text-sm font-semibold text-slate-600 mb-2">Gambar Saat Ini:</p>
                            <img src="{{ asset($item->image_path) }}" alt="{{ $item->name }}" class="image-preview">
                        </div>
                    @endif

                    <label class="form-file-label mt-4">
                        Ganti Gambar (JPG, PNG, GIF, WebP - Max 2MB)
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description" class="form-textarea" placeholder="Jelaskan item ini...">{{ old('description', $item->description) }}</textarea>
                    @error('description')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                </div>
