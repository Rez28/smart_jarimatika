@if (
    ...
    <nav x-data="{ open: false }" class="sticky top-0 z-50 bg-white border-b border-slate-200 shadow-sm font-['Fredoka']">
        ...
        <!-- Navbar Container -->
        <div class="max-w-full px-4 sm:px-6 py-3 nav-hud">
            <div class="flex items-center justify-between gap-4">
                <!-- Logo & Title -->
                <div class="flex items-center shrink-0">
                    <a href="{{ route('dashboard') }}"
                        class="flex items-center gap-1 transform hover:scale-110 transition-transform">
                        <img src="{{ asset('images/logo.png') }}" alt="Smart Jarimatika"
                ...
                <div class="hidden md:flex items-center space-x-1">
                    <a href="{{ route('dashboard') }}"
                        class="nav-link px-3 py-2 rounded-xl text-sm font-bold transition-colors duration-200 {{ request()->routeIs('dashboard') ? 'bg-sky-100 text-[#0284C7]' : 'text-slate-600 hover:text-slate-700 hover:bg-slate-50' }}">
                        🏠 Lobi
                    </a>
                    <a href="{{ route('jarimatika.belajar') }}"
                        class="nav-link px-3 py-2 rounded-xl text-sm font-bold transition-colors duration-200 {{ request()->routeIs('jarimatika.belajar') ? 'bg-lime-100 text-[#5e692a]' : 'text-slate-600 hover:text-slate-700 hover:bg-slate-50' }}">
                        📚 Belajar
                    </a>
                    <a href="{{ route('jarimatika.latihan') }}"
                        class="nav-link px-3 py-2 rounded-xl text-sm font-bold transition-colors duration-200 {{ request()->routeIs('jarimatika.latihan') ? 'bg-orange-100 text-[#c8790f]' : 'text-slate-600 hover:text-slate-700 hover:bg-slate-50' }}">
                        🎯 Latihan
                    </a>
                    ...
                    @auth
                        @if (auth()->user()->level >= 5)
                            <a href="{{ route('jarimatika.match') }}"
                                class="nav-link px-3 py-2 rounded-xl text-sm font-bold transition-colors duration-200 relative {{ request()->routeIs('jarimatika.match') ? 'bg-red-100 text-red-600' : 'text-slate-600 hover:text-slate-700 hover:bg-slate-50' }}">
                                ⚔️ Battle
                        ...
                        @else
                            <div class="px-3 py-2 rounded-xl text-sm font-bold text-gray-400 cursor-not-allowed flex items-center gap-1"
                    ...
                    <a href="{{ route('reward.leaderboard') }}"
                        class="nav-link px-3 py-2 rounded-xl text-sm font-bold transition-colors duration-200 {{ request()->routeIs('reward.leaderboard') ? 'bg-yellow-100 text-yellow-700' : 'text-slate-600 hover:text-slate-700 hover:bg-slate-50' }}">
                        🏆 Peringkat
                    </a>
                </div>
                ...
                <div class="md:hidden flex items-center">
                    <button @click="open = ! open"
                        class="inline-flex items-center justify-center p-2 rounded-lg text-slate-600 hover:text-slate-700 hover:bg-slate-100 focus:outline-none transition duration-150 ease-in-out">
        ...
        <div :class="{ 'block': open, 'hidden': !open }" class="hidden md:hidden border-t border-slate-200 bg-white">
            <div class="px-4 py-3 space-y-2">
                @auth
                    <div class="flex justify-around items-center mb-3 bg-slate-50 p-2 rounded-lg border border-slate-200">
            ...
            <div class="pt-2 pb-3 border-t border-slate-200 bg-slate-50 px-4">
                @auth
                    <div class="mb-3">
                ...
                @else
                    <div class="space-y-2">
                        <a href="{{ route('login') }}"
                            class="block text-center w-full bg-white border border-slate-200 py-2 rounded-lg text-xs font-bold text-slate-600">Masuk</a>
                        @if (Route::has('register'))
                            <a href="{{ route('register') }}"
